...
const existing: Word = { id: "word-1", bookId: book.id, original: "Oeuvre", normalizedOriginal: "oeuvre", translation: "obra editada", translationLanguage: "pt", context: null, page: null, encounterCount: 1, favorite: false, translationManuallyEdited: true, createdAt: now, updatedAt: now, lastEncounteredAt: now };
const translation: WordTranslation = { id: "translation-1", wordId: existing.id, targetLanguage: "pt", translation: existing.translation, manuallyEdited: true, createdAt: now, updatedAt: now };
...
describe("addWordToBook", () => {
  it("increments a normalized duplicate without translating or overwriting a manual translation", async () => {
    ...
    const store = {
      findBook: vi.fn().mockResolvedValue(book), findWord: vi.fn().mockResolvedValue(existingWithTranslations),
  ...
  it("translates and saves a new normalized word", async () => {
    const created = { ...existingWithTranslations, id: "word-2", original: "languid", normalizedOriginal: "languid", translation: "lânguido", translationManuallyEdited: false };
    const store = {
    ...
    expect(store.createWord).toHaveBeenCalledWith(expect.objectContaining({ normalizedOriginal: "languid", translation: "lânguido", translationLanguage: "pt" }));
  });
});
