...
const initial: SettingsData = { defaultSourceLanguage: "fr", defaultTargetLanguage: "en", showAllLanguages: false, appearance: "system" };
...
export default function SettingsPage() {
  ...
  const languages = languageOptions(form.showAllLanguages, [form.defaultSourceLanguage, form.defaultTargetLanguage]);
  return <main className="shell page settings-page">
    ...
    <form className="settings-card" onSubmit={save}>
      <section><h2>Languages</h2><p>Used to prefill new books and keep language menus focused.</p><div className="form-grid"><label>Source language<select value={form.defaultSourceLanguage} onChange={(e) => update("defaultSourceLanguage", e.target.value)}>{languages.map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></label><label>Translation language<select value={form.defaultTargetLanguage} onChange={(e) => update("defaultTargetLanguage", e.target.value)}>{languages.map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></label></div><label className="setting-toggle"><input type="checkbox" checked={form.showAllLanguages} onChange={(e) => update("showAllLanguages", e.target.checked)} /><span><strong>Show all supported languages</strong><small>Off shows Portuguese, French, English, and Spanish, plus languages already in use.</small></span></label></section>
